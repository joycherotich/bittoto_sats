...
const AchievementsDisplay: React.FC<AchievementsDisplayProps> = ({
  ...
  const { api, isParent } = useAuth();
  const [isLoading, setIsLoading] = useState(true);
  const [achievements, setAchievements] = useState<Achievement[]>([]);
  const [selectedCategory, setSelectedCategory] = useState<string>('all');

  // Use childId in your API calls
  useEffect(() => {
    const fetchAchievements = async () => {
      ...
      try {
        ...
        if (Array.isArray(achievementsData) && achievementsData.length > 0) {
          ...
          console.log('No achievements data or empty array, using mock data');
        ...
        toast({
          variant: 'warning',
          title: 'Using demo data',
          description: 'Error connecting to achievements API',
        });
      } finally {
        setIsLoading(false);
      }
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
          {filteredAchievements.map((achievement) => (
            <Card
              key={achievement.id}
              className={`${
                achievement.unlocked ? 'border-amber-500' : 'opacity-80'
              }`}
            >
